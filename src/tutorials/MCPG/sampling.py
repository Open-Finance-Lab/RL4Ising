...
def mcpg_sampling_ising(data,
    ...
    temp_max = energy_vec[idx]  # [total_mcmc_num]
    temp_max_info = graph_probs[:, idx]  # [nvar, total_mcmc_num]
    start_samps = start  # [nvar, chains]
    
    return temp_max, temp_max_info, start_samps, adv


'''def mcpg_sampling_ising(data, tensor_probs, probs, num_ls, change_times, total_mcmc_num):
    """
    Metropolis-based sampler for Ising energy:
        E(s) = - sum_{(i,j)} J_ij * s_i * s_j

    Returns:
      best_E       : Tensor [num_chains] of best (lowest) energies found
      best_config  : Tensor [nvar, num_chains] of corresponding spin configs
      start_samples: Tensor [num_chains, nvar] final samples to seed next epoch
      final_E      : Tensor [num_chains] of energies of the final samples
    """
    # device & initial samples: tensor_probs is [nvar, num_chains]
    device = data.edge_index.device
    samples = tensor_probs.t().clone()           # [num_chains, nvar]
    num_chains, nvar = samples.shape

    # edges & weights
    u, v = data.edge_index                                 # each [num_edges]
    w   = getattr(data, "edge_weight", None) or data.edge_attr  # [num_edges]

    # track best so far
    best_E      = torch.full((num_chains,), float('inf'), device=device)
    best_config = samples.clone()    # [num_chains, nvar]

    for _ in range(change_times):
        # --- compute current energy per chain ---
        x_u = samples[:, u]   # [num_chains, num_edges]
        x_v = samples[:, v]
        E_before = -(w * (x_u * x_v)).sum(dim=1)  # [num_chains]

        # --- propose one random flip per chain ---
        flips = torch.randint(0, nvar, (num_chains,), device=device)
        idx   = torch.arange(num_chains, device=device)

        # build proposed configurations
        samples_prop = samples.clone()
        samples_prop[idx, flips] *= -1

        # --- compute proposed energy ---
        x_u_prop = samples_prop[:, u]
        x_v_prop = samples_prop[:, v]
        E_after  = -(w * (x_u_prop * x_v_prop)).sum(dim=1)

        # --- Metropolis accept/reject ---
        delta = E_after - E_before
        prob_accept = torch.exp(-delta)
        accept = (E_after <= E_before) | (torch.rand(num_chains, device=device) < prob_accept)
        acc_idx = torch.nonzero(accept, as_tuple=True)[0]
        samples[acc_idx, flips[acc_idx]] *= -1

        # --- update best if improved ---
        better = E_after < best_E
        best_E[better] = E_after[better]
        best_config[better] = samples[better]

    # (optional) you could insert your local‐search num_ls loops here

    # final energies of each chain
    x_u_final = samples[:, u]
    x_v_final = samples[:, v]
    print(w.shape)
    print(x_u_final.shape)
    print(x_v_final.shape)
    final_E = -(w * (x_u_final * x_v_final)).sum(dim=1)

    # return in the order expected by mcpg_solver:
    #   temp_max (best_E), temp_max_info (best_config transposed),
    #   start_samples (samples), value (final_E)
    return (
        best_E,                     # [num_chains]
        best_config.t().contiguous(),  # [nvar, num_chains]
        samples,                    # [num_chains, nvar]
        final_E                     # [num_chains]
    )
